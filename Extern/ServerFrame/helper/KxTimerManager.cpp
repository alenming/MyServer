...
bool KxTimerList::insert(KxTimerObject* obj)
{
    ...
    else
    {
        while(NULL != pNode)
        {
            // Èç¹û¸Ã½ÚµãµÄ³¬Ê±Ê±¼ä´óÓÚÒª²åÈë½ÚµãµÄ³¬Ê±Ê±¼ä
...
void KxTimerList::update(const kxTimeVal& now)
{
    while(NULL != m_Head)
    {
        if (m_Head->checkTime(now))
        {
            ...
            // Èç¹ûÔÚonTimerÖÐ£¬¶Ôm_TimerÖ´ÐÐÁËstop²Ù×÷£¬remove»áÊ§°Ü£¨²»stopµÄ»°¼ÓÈëÆäËüÁÐ±íµÄ²Ù×÷»áÊ§°Ü£©
            // ¶ø¶ÔÓÚAglieListÖÐµÄTimer£¬Repeat±Ø¶¨Îª0
            // Ö»ÒªÊÇÔÚ±¾³¬Ê±¶ÓÁÐÖÐµÄTimer£¬¸ù¾ÝRepeat¾ö¶¨ÖØ¸´Ö´ÐÐµÄ´ÎÊý
