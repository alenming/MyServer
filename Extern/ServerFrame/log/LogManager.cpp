...
//#ifdef _DEBUG
    if (level >= m_LogLevel)
    {
        ...
        va_list ap;
        va_start(ap, log);
        vsnprintf(szBuf, MAX_BUF, log, ap);
        va_end(ap);
...
void LogManager::updateFormat(int level, const char* log, const char* file, const char* func, int line)
{
    ...
    if (m_DateOption || m_TimeOption)
    {
        ...
        if (m_DateOption)
        {
            sprintf(szBuf, "[%04d-%02d-%02d]", tmnow.tm_year + 1900, tmnow.tm_mon, tmnow.tm_mday);
            m_StringFormat += szBuf;
        }

        if (m_TimeOption)
        {
            sprintf(szBuf, "[%02d:%02d:%02d]", tmnow.tm_hour, tmnow.tm_min, tmnow.tm_sec);
            m_StringFormat += szBuf;
        }
    }
