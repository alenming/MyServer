...
int XML::x_ParseError( const char* szError, const char* szName )
{
	...
	{
		char szFormat[300];
		//std::sprintf( szFormat,sizeof(szFormat),szError, szName );
        std::sprintf( szFormat,szError, szName );
		m_strError = szFormat;
	}
	else
		m_strError = szError;
...
string XML::x_GetToken( const XML::TokenPos& token ) const
{
	...
	if ( token.nL > token.nR )
		return "";
	return m_strDoc.substr(token.nL, (token.nR - token.nL + (((unsigned int)(token.nR)<m_strDoc.size()) ? 1 : 0)));
}
...
string XML::x_GetAttrib( int iPos, const char* szAttrib ) const
{
	...
	if ( szAttrib && x_FindAttrib( token, szAttrib ) )
		return x_TextFromDoc(token.nL, token.nR - (((unsigned int)(token.nR)<m_strDoc.size()) ? 0 : 1));
	return "";
}
...
bool XML::x_SetAttrib( int iPos, const char* szAttrib, int nValue )
{
	...
	char szVal[25];
	//std::sprintf(szVal, sizeof(szVal), "%d", nValue);
    std::sprintf(szVal, "%d", nValue);
